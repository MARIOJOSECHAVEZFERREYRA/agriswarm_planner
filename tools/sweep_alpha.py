"""Sweep the UAV-wait penalty weight (alpha) across fixtures under B2a.

Hypothesis under test: under the parked-UGV kinematic model (B2a),
alpha=1.0 over-penalises UAV wait, so the greedy keeps the UGV at its
starting position (where wait=0) and makes the drone fly all the
deadheads. Lowering alpha should let the greedy prefer candidates the
UGV can advance to, trading wait time for shorter drone deadheads.

For each alpha, rebinds RendezvousPlanner.__init__.__defaults__ so that
BOTH the grid search simulator and plan_dynamic_cycles see the new
weight. Then runs the full pipeline and measures cuts.
"""
...
def _set_alpha(alpha_value):
    RendezvousPlanner.__init__.__defaults__ = (float(alpha_value), 0.5, 0.3, 50.0)
...
def main():
    header = (
        f"  {'alpha':>5}  {'fixture':<24}  {'ang':>4}  {'cuts':>4}  "
